...
<script type="text/javascript">
	function show(next, total) {
		...
		document.forms[0].action = "/ssm/email/selectNextPageByRecycle?page=" + next;
			...
			<table width="90%" border="1" cellpadding="0" cellspacing="0" style="border: thin solid #666; margin-top: 20px">
				...
				<c:forEach var="e" items="${list}">
					<tr>
						...
						<td align="center">
							<button
								onclick="if(window.confirm( '确定要还原[${e.title}]吗?')==true){
												window.location.href='/ssm/email/reduction?EId=${e.eId}&r=${sessionScope.user.nickname}'}">还原</button>

							<button
								onclick="if(window.confirm( '确定要删除[${e.title}]吗?')==true){
												window.location.href='/ssm/email/delete?EId=${e.eId}&r=${sessionScope.user.nickname}'}">删除</button>
						</td>
					</tr>
				</c:forEach>
			</table>
